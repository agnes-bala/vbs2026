      ...
      .of(
        Yup.object().shape({
          ...
          dateOfBirth: Yup.string()
            ...
            .test(
              "dateOfBirth",
              "child age must be 50 years or younger",
              (value) => {
                return moment().diff(moment(value, "yyyy-MM-dd"), "years") < 50;
              },
            )
            .test(
              "dateOfBirth",
              "child age must be 1 months or more",
              (value) => {
                return moment().diff(moment(value, "yyyy-MM-dd"), "days") > 30;
              },
            )
  ...
  const formik = useFormik({
    ...
    onSubmit: async (childrenInfo) => {
      ...
      let resp = await axios.get(
        `${jrmClientUrl}jrms/v1/partners/${pId}/childreninfo`,
        { headers: config },
      );
    ...
    <>
      {isLoading ? (
        <Box sx={{ position: "relative" }}>
          <LinearProgress size={80} thickness={20} value={100} />
        </Box>
      ) : (
          ...
          <Form noValidate onSubmit={handleSubmit}>
            <Grid item xs={12} md={6} lg={6}>
              <Stack spacing={3}>
                <Stack spacing={3}>
                  <FormControl>
                    <FormLabel>Relationshippp to a Child/Children</FormLabel>
                    <RadioGroup
                      row
                      name="relationship"
                        ...
                        {({ push, remove }) => (
                          <div>
                            {values.children.map((p, index) => {
                              ...
                              const toucheddateOfBirth = getIn(
                                touched,
                                dateOfBirth,
                              );
                              const errordateOfBirth = getIn(
                                errors,
                                dateOfBirth,
                              );
