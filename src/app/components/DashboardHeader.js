...
export default function DashboardHeader() {
  ...
  const isActiveRoute = (path) => path === '/' ? pathname === path : pathname.startsWith(path);
    ...
    <Box sx={{ width: isSmallMobile ? '85vw' : 320, p: 3, display: 'flex', flexDirection: 'column', height: '100%', alignItems: 'center' }}>
      ...
      <Stack spacing={1} sx={{ flex: 1, width: '100%' }}>
        {navItems.map((item, index) => (
          item.external ? (
            <DrawerItem key={index} component="a" href={item.path} target="_blank" rel="noopener noreferrer" onClick={handleDrawerToggle}
              startIcon={<Box component="img" src={item.img} alt={item.label} sx={{ width: 30, height: 30 }} />}
              sx={{ justifyContent: 'center' }}>{item.label}</DrawerItem>
          ) : (
            <DrawerItem key={index} component={Link} href={item.path} active={isActiveRoute(item.path) ? 1 : 0} onClick={handleDrawerToggle}
              startIcon={<Box component="img" src={item.img} alt={item.label} sx={{ width: 30, height: 30 }} />}
              sx={{ justifyContent: 'center' }}>{item.label}</DrawerItem>
          )
        ))}
      ...
      <Box sx={{ mt: 'auto', pt: 3, width: '100%' }}>
        {!user ? (
          <ActionButton onClick={() => { setPopup(true); setMobileOpen(false); }} variant="contained" fullWidth>Login</ActionButton>
        ) : (
  ...
  return (
    <>
      <AppBar position="sticky" sx={{ backgroundColor: BLUE.main, clipPath: { xs: 'none', md: scrolled ? 'none' : "ellipse(100% 100% at 50% 0%)" }, boxShadow: scrolled ? 4 : 1, transition: 'all 0.3s ease', top: 0, zIndex: theme.zIndex.appBar }}>
        <Container maxWidth="xl">
          <Toolbar sx={{ justifyContent: "space-between", minHeight: { xs: 64, sm: 70, md: 80 }, px: { xs: 1, sm: 2 }, alignItems: "center" }}>
            ...
            {isMobile ? (
              ...
              <Box sx={{ display: "flex", gap: 2, alignItems: "center" }}>
                <Stack direction="row" spacing={2} alignItems="center">
                  {navItems.map((item, index) => (
                    item.external ? (
                      <Link key={index} href={item.path} target="_blank" rel="noopener noreferrer" underline="none">
                        <NavItem><Box component="img" src={item.img} alt={item.label} sx={{ width: 40, height: 40 }} /><Typography sx={{ fontWeight: "bold", fontSize: '0.9rem', color: 'white' }}>{item.label}</Typography></NavItem>
                      </Link>
                    ) : (
                      <Link key={index} href={item.path} style={{ textDecoration: "none" }}>
                        <NavItem active={isActiveRoute(item.path) ? 1 : 0}><Box component="img" src={item.img} alt={item.label} sx={{ width: 40, height: 40 }} /><Typography sx={{ fontWeight: "bold", fontSize: '0.9rem' }}>{item.label}</Typography></NavItem>
                      </Link>
                    )
                  ))}
                </Stack>
                <Stack direction="row" spacing={0.5} sx={{ mx: 1 }}>
                  {socialLinks.map((social, index) => (
                    <Link key={index} href={social.href} target="_blank" rel="noopener noreferrer" underline="none">
                      <SocialIconButton size="small" aria-label={social.label}><social.icon /></SocialIconButton>
                    </Link>
                  ))}
                </Stack>
                {pathname !== '/dashboard/myapp' && (
                  <Button component={Link} href="/dashboard/myapp" variant="contained" sx={{ backgroundColor: "white", color: BLUE.main, fontWeight: "bold", ":hover": { backgroundColor: "rgba(255,255,255,0.9)", color: BLUE.dark }, animation: "pulse 1.5s ease-in-out infinite", "@keyframes pulse": { "0%": { transform: "scale(1)" }, "50%": { transform: "scale(1.1)" }, "100%": { transform: "scale(1)" } } }}>Dashboard</Button>
                )}
                    ...
                    <>
                      ...
                      <AccountMenu anchorEl={anchorEl} open={Boolean(anchorEl)} onClose={handleAccountClose} anchorOrigin={{ vertical: 'bottom', horizontal: 'right' }} transformOrigin={{ vertical: 'top', horizontal: 'right' }}>
                        <Box sx={{ px: 2, py: 1.5, display: 'flex', alignItems: 'center', gap: 1.5 }}>
                          ...
                          <Box><Typography variant="subtitle2">{userName}</Typography><Typography variant="caption" color="text.secondary">{userEmail}</Typography></Box>
                        </Box>
                        <Divider />
                        <MenuItem component={Link} href="/dashboard/myapp" onClick={handleAccountClose}><ListItemIcon><DashboardIcon fontSize="small" sx={{ color: BLUE.main }} /></ListItemIcon><ListItemText>Dashboard</ListItemText></MenuItem>
                        <MenuItem component={Link} href="/dashboard/user/profile" onClick={handleAccountClose}><ListItemIcon><PersonIcon fontSize="small" sx={{ color: BLUE.main }} /></ListItemIcon><ListItemText>Profile</ListItemText></MenuItem>
                        <Divider />
                        <MenuItem onClick={handleLogout}><ListItemIcon><LogoutIcon fontSize="small" sx={{ color: BLUE.main }} /></ListItemIcon><ListItemText>Logout</ListItemText></MenuItem>
                      </AccountMenu>
                    </>
                  )}
                </Stack>
      ...
      {isMobile && (
        <Box sx={{ position: 'fixed', bottom: 0, left: 0, right: 0, backgroundColor: 'background.paper', boxShadow: '0px -2px 10px rgba(114, 72, 187, 0.15)', display: 'flex', justifyContent: 'center', padding: '8px 4px', zIndex: theme.zIndex.appBar - 1, backdropFilter: 'blur(8px)', background: 'rgba(255,255,255,0.98)', borderTop: `2px solid ${BLUE.main}` }}>
      ...
      {/* Mobile Account Menu */}
      <Menu anchorEl={mobileAnchorEl} open={Boolean(mobileAnchorEl)} onClose={handleMobileAccountClose} anchorOrigin={{ vertical: 'top', horizontal: 'center' }} transformOrigin={{ vertical: 'bottom', horizontal: 'center' }} PaperProps={{ sx: { mt: -2, width: 220, borderRadius: 2, boxShadow: `0 8px 32px ${BLUE.transparent.dark}` } }}>
        <Box sx={{ px: 2, py: 1.5, display: 'flex', alignItems: 'center', gap: 1.5 }}>
          <Avatar sx={{ bgcolor: BLUE.main, width: 40, height: 40 }}>{userAvatar}</Avatar>
          <Box><Typography variant="subtitle2">Hello Ablazer!</Typography></Box>
        </Box>
        <Divider />
        <MenuItem component={Link} href="/dashboard/myapp" onClick={handleMobileAccountClose}><ListItemIcon><DashboardIcon fontSize="small" sx={{ color: BLUE.main }} /></ListItemIcon><ListItemText>Dashboard</ListItemText></MenuItem>
        <MenuItem component={Link} href="/dashboard/user/profile" onClick={handleMobileAccountClose}><ListItemIcon><PersonIcon fontSize="small" sx={{ color: BLUE.main }} /></ListItemIcon><ListItemText>Profile</ListItemText></MenuItem>
        <Divider />
        <MenuItem onClick={handleLogout}><ListItemIcon><LogoutIcon fontSize="small" sx={{ color: BLUE.main }} /></ListItemIcon><ListItemText>Logout</ListItemText></MenuItem>
      </Menu>

      {/* Mobile Drawer */}
      <Drawer anchor="right" open={mobileOpen} onClose={handleDrawerToggle} PaperProps={{ sx: { borderRadius: '20px 0 0 20px', display: 'flex', alignItems: 'center' } }}>{drawer}</Drawer>

      {/* Login Popup */}
      {popup && (
        <Card sx={{ width: { xs: "90%", sm: "400px" }, padding: 4, position: "fixed", top: "50%", left: "50%", transform: "translate(-50%, -50%)", zIndex: 1400, boxShadow: `0 8px 32px ${BLUE.transparent.dark}`, borderRadius: '16px' }}>
          <LoginForm />
          <IconButton onClick={() => setPopup(false)} sx={{ position: "absolute", top: 10, right: 10, cursor: "pointer", color: BLUE.main, '&:hover': { backgroundColor: BLUE.transparent.main } }}><CloseIcon /></IconButton>
        </Card>
      )}
